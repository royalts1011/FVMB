...
def crop_image(input_framed,src):
    ...
    mask2 = (img_out[:,:,0]-120).float().pow(2)+(img_out[:,:,1]-102).float().pow(2)+(img_out[:,:,2]-201).float().pow(2)
    x_crop = F.avg_pool1d(F.avg_pool1d(torch.exp(-0.0005*mask2).sum(0).view(1,1,-1),3,padding=1,stride=1),3,padding=1,stride=1)
    x_crop = (x_crop.squeeze()==F.max_pool1d(x_crop,9,stride=1,padding=4).squeeze()).float()*x_crop.squeeze()
    y_crop = F.avg_pool1d(F.avg_pool1d(torch.exp(-0.0005*mask2).sum(1).view(1,1,-1),3,padding=1,stride=1),3,padding=1,stride=1)
    y_crop = (y_crop.squeeze()==F.max_pool1d(y_crop.view(1,1,-1),9,stride=1,padding=4).squeeze()).float()*y_crop.squeeze()
...
def extract_scribbles(cropped_out):
    ...
    mask_colour = torch.max_pool2d(1-mask_colour0.float().unsqueeze(0).unsqueeze(0),5,stride=1,padding=2).squeeze()==0
    label = ((cropped_out[:,:,:3].reshape(-1,3)[mask_colour.view(-1),:])*torch.Tensor([1,256,256**2]).long().view(1,3)).sum(1)
    ...
    labels.view(-1)[mask_colour.view(-1)] = (torch.argmin((cropped_out.reshape(-1,1,3)[mask_colour.view(-1),:,:].long()-unique_rgb.view(1,-1,3)).pow(2).sum(2),1)+1)
...
def frame_image(img):
    #print(img.shape)
    img[:,:,0] = img[:,:,1]; img[:5,:,0] = 120; img[-5:,:,0] = 120; img[:,:5,0] = 120; img[:,-5:,0] = 120;
    img[:,:,2] = img[:,:,1]; img[:5,:,2] = 201; img[-5:,:,2] = 201;  img[:,:5,2] = 201; img[:,-5:,2] = 201;
    img[:5,:,1] = 102; img[-5:,:,1] = 102; img[:,:5,1] = 102; img[:,-5:,1] = 102;
    return img
